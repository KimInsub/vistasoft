function [range TolFun] = rmSearchFit_range_temporal_linear(params,model,data)
...
if gridSigmas(closestvalue) - expandRange < 0
    range.lower(3,:) = gridSigmas(1);
end

% exponent set to one
fieldnum=size(range.start, 1)+1;
range.start(fieldnum,:) = ones(size(model.exponent));
range.upper(fieldnum,:) = ones(size(model.exponent));
range.lower(fieldnum,:) = ones(size(model.exponent));
